...
public class Skyscrapers {
    ...
    private static final String digits = "123456";
    ...
    private void parseGrid() {
        ...
        for (int i = 0; i < 24; i++) {
            int clue = clues[i];
            if (clue == 1) {
                assign(values, clueUnits.get(i).get(0), "6");
            } else if (clue == 6) {
                for (int j = 0; j < 6; j++) {
    ...
    private void initClueUnits() {
        ...
        for (int i = 6; i < 12; i++) {
            clueUnits.put(i, cross(rows.toCharArray()[i - 6] + "", "654321"));
        }

        for (int i = 12; i < 18; i++) {
            clueUnits.put(i, cross("FEDCBA", cols.toCharArray()[17 - i] + ""));
        }

        for (int i = 18; i < 24; i++) {
            clueUnits.put(i, cross(rows.toCharArray()[23 - i] + "", cols));
        }
    }
    ...
    private void display(Map<String, String> values) {
        ...
        for (int i = 0; i < 6; i++) {
            System.out.printf("%6s | ", clues[i]);
        }
        System.out.printf("%6s | \n", "");

        for (int i = 0; i < squares.length; i++) {
            if (i % 6 == 0) {
                System.out.printf("| %6s | ", clues[23 - (i / 6)]);
            }

            System.out.printf("%6s | ", values.get(squares[i]));
    ...
    public static int[][] solvePuzzle(int[] clues) {
        ...
        for (int i = 0; i < sc.squares.length; i++) {
            solved[i / 6][i % 6] = Integer.parseInt(values.get(sc.squares[i]));
        }
